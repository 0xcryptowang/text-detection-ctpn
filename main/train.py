...
tf.flags.DEFINE_integer('max_steps', 60000, '')
tf.flags.DEFINE_integer('decay_steps', 30000, '')
...
tf.flags.DEFINE_string('gpu', '0', '')
tf.flags.DEFINE_string('checkpoint_path', '../data/model/checkpoints/', '')
tf.flags.DEFINE_string('logs_path', '../data/tmp/logs/', '')
tf.flags.DEFINE_string('pre_trained_model_path', '../data/model/vgg/vgg_16.ckpt', '')
tf.flags.DEFINE_boolean('restore', True, '')
tf.flags.DEFINE_integer('save_checkpoint_steps', 2000, '')
FLAGS = tf.flags.FLAGS


def main(argv=None):
    global variable_restore_op
    ...
    if FLAGS.pre_trained_model_path is not None:
        variable_restore_op = slim.assign_from_checkpoint_fn(FLAGS.pre_trained_model_path,
    ...
    with tf.Session(config=config) as sess:
        if FLAGS.restore:
            ...
            restore_step = int(os.path.splitext(os.path.split(ckpt)[-1].split('_')[-1])[0])
